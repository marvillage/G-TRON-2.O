...
def predict_hazard_and_value(data, state, device_type, xgb_model, catboost_model):
    ...
    if xgb_model is None or catboost_model is None:
        if 'hazard_factor' in filtered_data.columns and 'recycling_rate' in filtered_data.columns:
            ...
            hazard_score = recent_data.get('hazard_factor', 0.5) * 10  # Scale to 0-10
            avg_quantity = filtered_data['e_waste_quantity'].mean()
            econ_value = avg_quantity * 1000  # Simple estimation
            return hazard_score, econ_value
        else:
            # Generate random but consistent values if needed columns don't exist
            hazard_score = 5.0  # Mid-range value
            econ_value = filtered_data['e_waste_quantity'].mean() * 1000
            return hazard_score, econ_value
    ...
    try:
        ...
        # Make predictions
        hazard_score = xgb_model.predict(user_input)[0]
        econ_value = catboost_model.predict(user_input)[0]
        
        return hazard_score, econ_value
    except Exception as e:
        st.error(f"Prediction error: {str(e)}")
        return 5.0, filtered_data['e_waste_quantity'].mean() * 1000  # Fallback values
...
xgb_model, catboost_model = load_models()
        ...
        if hazard_score is None or econ_value is None:
            ...
            with col1:
                st.metric("Predicted Hazard Score", f"{hazard_score:.2f}")
            ...
            with col2:
                st.metric("Estimated Economic Value", f"₹{econ_value:,.2f}")
                # Historical trend
