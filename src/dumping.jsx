...
import firebaseConfig from '../firebase_config2.json';
...
function DumpingPage() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const storageRef = ref(storage, `reports/${Date.now()}-${imageFile.name}`);
  ...
  const getStatusIcon = (status) => {
    switch (status) {
      case 'Pending':
        return '/images/pending_marker.png';
      case 'Verified':
        return '/images/verified_marker.png';
      case 'Resolved':
        return '/images/resolved_marker.png';
      default:
